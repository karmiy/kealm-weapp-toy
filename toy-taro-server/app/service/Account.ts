...
export default class Account extends Service {
  ...
  public async getAccountTypeList(mode?: string) {
    ...
    const accountModeCondition = !ctx.helper.isEmpty(mode)
      ? { account_mode: mode }
      : {};
    ...
    return list as any as Promise<
      Array<{ id: number; name: string; account_mode: number }>
    >;
  ...
  public async getAccountRecordById(conditions: {
    id: number;
    open_id: string;
  }) {
    const { ctx, app } = this;
  ...
  public async deleteAccountRecordById(conditions: {
    id: number;
    open_id: string;
  }) {
    const { ctx } = this;
  ...
  public async getAmountStatistics(conditions: {
    ...
    const startDate = ctx.helper.getFirstDayOfMonth(
      year,
      isGroupByDay ? month ?? 1 : 1
    );

    const endDate = ctx.helper.getLastDayOfMonth(
      year,
      isGroupByDay ? month ?? 12 : 12
    );
    ...
    const groups: Record<
      string,
      { income: number; expenditure: number; total: number }
    > = Object.create(null);
    ...
    data.forEach(({ create_time, income, expenditure }) => {
      groups[create_time] = {
        ...
        total: ctx.helper.add(Number(income), Number(expenditure)),
      };
    });
